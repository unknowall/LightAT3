...
namespace LightCodec.util
{
    public class CodecUtils
    {
        ...
        private static unsafe short convertSampleFloatToInt16(float sample)
        {
            sample = sample < -1.0f ? -1.0f : (sample > 1.0f ? 1.0f : sample);
            const float Scale = 32768f;
            int ival = (int)(sample * Scale + (sample >= 0 ? 0.5f : -0.5f));
            ival = (ival & ~0x7FFF) == 0 ? ival : (ival < 0 ? -32768 : 32767);
        ...
        //{
        //    return Math.Min(Math.Max((int)(sample * 32768f + 0.5f), -32768), 32767) & 0xFFFF;
        //}
